...
from footypreds.config import Settings  # noqa: E402
from footypreds.provider import FlashScore  # noqa: E402
from footypreds.store import Store  # noqa: E402
...
async def main():
    ...
    try:
        ...
        eligible = [m for m in matches if m.status == "scheduled"]
        preferred = [m for m in eligible if m.country in ("England", "Spain", "Romania")]
        if eligible:
            match = (preferred or eligible)[0]
            history, _, rejected = await provider.head_to_head(match)
            store.save_matches(history)
            from footypreds.engine import analyze

            prediction = analyze(match, store.matches())
            print(f"H2H: {len(history)} results; rejected={rejected}")
            print(f"Model sample: {prediction['sample']}; grade={prediction['grade']}")
    finally:
            ...
            init = await client.post(
                ...
                json={
                    ...
                    "params": {
                        ...
                        "capabilities": {},
                        "clientInfo": {"name": "footypreds-check", "version": "8.0"},
                    },
                },
            )
