...
async function loadTicketHistory(scope, sports) {
  ...
  $('#history-summary').innerHTML = summary.length ? summary.map(s => `<div class="card summary-card">
      <div class="sc-head"><span class="ticket-badge small">x${esc(Number(s.target) >= 10 ? Math.round(s.target) : s.target)}</span><b>${esc(s.tickets)} bilete</b></div>
...
async function loadLedger(scope) {
  ...
  $('#ledger').innerHTML = `
    <div class="kpi-grid kpi-4">
      ...
      ${kpi('Acoperire', pct(metrics.coverage), `din ${esc(metrics.total_matches ?? 0)} meciuri analizate`)}
    </div>
    <div class="card panel"><h2 class="panel-title">Știe modelul cât de sigur este?</h2><p class="small muted">Probabilitatea declarată (gri) față de cât s-a câștigat efectiv (culoare), pe benzi.</p>
      ${(metrics.calibration || []).length ? `<div class="calibration">${metrics.calibration.map(b => `<div class="cal-band">
          <b>${esc(b.range)}</b><span class="muted small">${esc(b.count)} selecții</span>
...
const METHOD = {
  football: ['Fotbal', [
    ...
    ['Piața', 'Când există cote 1X2, probabilitățile modelului sunt combinate cu cele ale caselor, fără marjă (pondere aleasă pe validare).'],
  ]],
  basketball: ['Baschet', [
...
const METHOD_COMMON = [
  ['Bilete AI', 'Pentru o cotă țintă, un optimizator exact caută combinația de selecții (cel mult una pe meci, niciodată aceeași echipă de două ori) cu cea mai mare probabilitate combinată și cota totală în intervalul 0.93–1.12 × țintă. Selecțiile au note A–C, cote reale între 1.08 și 4.0 și nu pot fi rambursate.'],
  ...
  ['Simulator', 'Walk-forward orb: pentru fiecare zi, modelul vede doar rezultatele din zilele anterioare, fixează miza și abia apoi află scorurile, decontate la cotele istorice.'],
];

const LIMITATIONS = [
  'Simulările arată că biletele AI și selecțiile simple pierd bani pe toate seturile de fotbal testate și merg mai slab decât pariul pe favoritul casei.',
  'Selecțiile „sigure” de fotbal sunt adesea peste/sub 2.5 goluri, unde modelul este prea încrezător (2025-26: sub 2.5 estimat 63%, observat 52%).',
...
function benchmarkView(report) {
  ...
  return `<p class="small muted">Sezonul ${esc(report.holdout_season)}: ${esc(report.holdout_matches)} meciuri, evaluate cronologic, fără ca modelul să vadă scorurile.</p>
